...
void* hostapd_ctrl_new(const char* if_name)
{
    ...
    if (snprintf(ctrl->sa_dst.sun_path, sizeof(ctrl->sa_dst.sun_path),
                 "/var/run/hostapd/%s", if_name) >= sizeof(ctrl->sa_dst.sun_path))
        goto hostapd_ctrl_new_error_3;
...
*/
int hostapd_ctrl_get_stations(const void* handle,
                              hostapd_station_info_t* station_info,
                              int* station_entries)
{
    ...
    status = sendto(ctrl->fd, request, request_len, 0,
                    (struct sockaddr*)&(ctrl->sa_dst), sizeof(ctrl->sa_dst));
...
int hostapd_ctrl_find_by_mac(const hostapd_station_info_t* station_info,
                             int station_entries,
                             const uint8_t* mac)
{
    int i;
